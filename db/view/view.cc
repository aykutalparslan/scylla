    ...
    for (auto& mut : mutations) {
        ...
        if (paired_endpoint) {
            ...
            if (is_endpoint_local && pending_endpoints.empty()) {
                ...
                // writes but mutate_locally() doesn't, so until it is fixed
                // to do that, we need to do this here.
                ++stats.writes;
                fs->push_back(service::get_local_storage_proxy().mutate_locally(mut).then_wrapped([&stats] (auto&& fut) {
                    --stats.writes;
                    if (fut.failed()) {
                        vlogger.error("Error applying local view update: {}", fut.get_exception());
